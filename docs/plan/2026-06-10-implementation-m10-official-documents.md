...
> 작성 2026-06-10(수). 상태: **구현 완료 — 8단계 E2E 통과**
> 사용자 결정: 일반 양식 2종으로 시작(실양식 확보 시 템플릿만 교체), 설계대로 진행.

## 검증 결과
- 백엔드 테스트 99개 PASS(신규 13: 발급 401/토큰 43자/화이트리스트, 열람 401·404·10회 잠금·CSRF,
  직인 업로드→view base64·비번 거부·스푸핑·타입·1MB).
- **브라우저 8단계 E2E 전부 통과**: ① 갤러리(축소 렌더 썸네일) ② 양식 클릭 ③ 입력→실시간 미리보기
  ④ 비밀번호 모달→발급 ⑤ URL 모달(43자 토큰) ⑥ 공개 URL 비밀번호 게이트(오류 메시지/정답 통과)
  ⑦ 공문 렌더+직인 삽입 버튼 ⑧ 직인 PNG 업로드 → (인) 위 오버레이 합성(날인 후 버튼 소멸·인쇄만).
- 보안 리뷰 반영: **HIGH-1 매직 바이트 검증**(Content-Type 스푸핑 차단+테스트),
  **MED-1 프론트 contentType 화이트리스트**, **MED-3 잠금 카운터 비관적 락**(PESSIMISTIC_WRITE).
- 보류·거부(사유):
  - HIGH-2(multipart 한도 1MB 통일) **거부** — 전역 한도는 CSV 가져오기 2MB 와 공유. 1MB 비즈니스
    검증과의 이중 계층은 의도된 방어이며 1~2MB 수신 후 거부 비용은 사내툴에서 무시 가능.
  - MED-2(발급 rate limit)·MED-4(CSP 도입) — 사내툴 수용/전역 영향 커서 보류(Tier 3 후보로 기록).
