...
> 작성 2026-06-10(수). 상태: **구현 완료 — 검증 통과**. 기능개선 백로그 2번(엑셀 대체 목적 직결).

## 검증 결과
- 백엔드 테스트 72개 PASS(신규 4: 401/BOM·헤더·행/부서 필터/이스케이프+수식 인젝션 가드).
- 프론트 빌드 + Vitest 35개 PASS.
- 브라우저 E2E(localtest): 연락처 추가 → "CSV 내보내기" 클릭 → 토스트 → **다운로드 파일 바이트 검증**
  (BOM EF BB BF + `부서,이름,이메일\r\n영상의학과,홍길동,hong@amc.seoul.kr\r\n` 정확 일치).
